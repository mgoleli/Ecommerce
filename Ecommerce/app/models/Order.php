<?php 
    ...
    class Order extends Controller{
        ...
        public function getAllStatus(){ 
            $this->db->query("SELECT os.name, os.order_status_id FROM order_status os
            ");
             $orderStatus = $this->db->resultSet();
             if($orderStatus){
                 return $orderStatus;
             }else {
                 return false;
        ...
        public function statusUpdate($id,$order_status_id){
            //print_r($id); exit(); 
            $this->db->query("UPDATE c_order_details SET order_status_id=$order_status_id  WHERE $id");
           //print_r("UPDATE c_order SET order_status_id=$order_status_id  WHERE $id"); exit();
            $this->db->bind(':order_id',$id);
            $this->db->bind(':order_status_id',$order_status_id);
        ...
        public function getAllOrderDetalails($id){
            $this->db->query("SELECT od.*, os.name  FROM c_order_details od LEFT JOIN order_status os ON (od.order_status_id = os.order_status_id)
            WHERE order_id=:order_id");
            $this->db->bind(':order_id',$id);
        ...
        public function getUserOrderDetalails($user){
            $this->db->query("SELECT * FROM c_order_details od LEFT JOIN order_status os ON (od.order_status_id = os.order_status_id) 
            WHERE user=:user");
            $this->db->bind(':user',$user);
            return $this->db->resultSet();
        }
